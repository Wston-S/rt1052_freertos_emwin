...
static void _cbSelected(WM_MESSAGE * pMsg) {
  WM_HWIN hWin;
  int Index, xSize, yPos, ySizeFont, Id;
  const GUI_BITMAP * pBm;
  BUTTON_Handle bt_handle;
  hWin = pMsg->hWin;
  (void)Id;
  switch (pMsg->MsgId) {
  case WM_CREATE:
		bt_handle=BUTTON_CreateEx(679,409,120,70,hWin,WM_CF_SHOW,0,GUI_ID_USER+0);
		BUTTON_SetText(bt_handle,"BACK");
  break;
...
static void _CreateSelected(int Index, WM_HWIN hWin) {
  ...
  hWinBase = WM_GetParent(WM_GetParent(hWin)); //获取桌面句柄 hWinBase
...
//    return;
//  }
  WM_MOTION_Enable(1); //启用 emWin 的移动支持
  BUTTON_SetReactOnLevel();//防止按键误点...
  //
