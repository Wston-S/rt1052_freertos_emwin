...
//使用此参数,使得屏幕只支持单点(只针对传入emwin的(x,y)参数)，0表示没有touch，1表示touch
...
const uint8_t CTP_CFG_GT911[] =  {
  0x00,0x20,0x03,0xe0,0x01,0x05,0x3d,0x00,0x01,0x08,
...
uint8_t CTP_CFG_GT917S[] ={ 
  ...
  0x10,0x00,0x32,0xA2,0x07,0x64,0x53,0xB6,0x00
};
...
/* 触摸IC类型默认为5寸屏的ic */
TOUCH_IC touchIC = GT9157;  //实测为 917S_0200		
...
static void GTP_Touch_Up( int32_t id)
{
	...
	/*emwin相关*/
	Pid_State.Pressed = 0;
	GUI_PID_StoreState(&Pid_State);	/* 释放 */
...
int32_t GTP_Init_Panel(void)
{
  int32_t ret = -1;
	...
	/*emwin 获取(x,y)相关，这里.Layer=0表示图层1*/
	Pid_State.Layer=0;
