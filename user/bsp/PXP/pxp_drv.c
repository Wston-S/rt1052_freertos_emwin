...
const SURFACE *srcSurf,int src_x,int src_y,int src_w,int src_h)
{
	...
	PXP_WaitRdy();
 	...
 	//ps_cfg.pixelFormat =kPXP_PsPixelFormatRGB565;
	//ps_cfg.bufferAddr  =(uint32_t)bits;
	ps_cfg.pitchBytes  =srcSurf->WidthBytes; //两个垂直相邻像素之间的字节数
	ps_cfg.swapByte =FALSE;  	//不交换
	PXP_SetProcessSurfaceBufferConfig(PXP,&ps_cfg); //设置进程表面输入缓冲区配置 

	PXP_SetProcessSurfaceBackGroundColor(PXP,0x000000);//设置PS的背景色
	PXP_SetProcessSurfacePosition(PXP,0,0,dst_w-1,dst_h-1); //在输出缓冲区中设置Process表面位置
	PXP_SetProcessSurfaceScaler(PXP,src_w,src_h,dst_w,dst_h); //设置流程表面缩放器配置。
	PXP_SetAlphaSurfacePosition(PXP, 0, 0, 0, 0); // 在输出缓冲区中设置表面的 Alpha 位置

	//out_cfg.pixelFormat =kPXP_OutputPixelFormatRGB565;
	out_cfg.width =dst_w;
	out_cfg.height =dst_h;
	out_cfg.pitchBytes =dstSurf->WidthBytes; //两个垂直相邻像素之间的字节数
	//out_cfg.buffer0Addr =(uint32_t)gdev_16bpp_set_addr(pSurf,x,y);
	out_cfg.buffer1Addr =(uint32_t)0;
	out_cfg.interlacedMode =kPXP_OutputProgressive;  //隔行输出模式:所有数据以累进格式写入输出缓冲区0
	PXP_SetOutputBufferConfig(PXP,&out_cfg);
